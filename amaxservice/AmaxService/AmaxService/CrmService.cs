...
namespace AmaxService
{
    ...
    public class CrmService : ICrmService
    {
        ...
        public Stream Login(Stream obj)
        {
            try
            {
                Dictionary<string, Object> parameter = JsonConvert.DeserializeObject<Dictionary<string, object>>(new StreamReader(obj).ReadToEnd());

                string connection_str = "";
                Dictionary<string, object> parameterDictionary = new Dictionary<string, object>();
                using (DbAccess db = new DbAccess(getControllDbConnectionString()))
                {
                    parameterDictionary.Add("OrgId", parameter["OrgId"]);
                    ...
                    if (connection_str != "")
                        using (DbAccess da = new DbAccess(connection_str))
                        {
                            ...
                            parameterDictionary.Add("fname", parameter["UserName"]);
                            parameterDictionary.Add("pass", parameter["Password"]);
        ...
        public Stream AmaxReportingService(Stream Parameter)
        {
            if (IsValid)
                try
                {
        ...
        public Stream ExecuteJson(Stream Parameter)
        {
            if (IsValid)
                try
                {
        ...
        public Stream GetTreeData(Stream Parameter)
        {
        ...
        public Stream SendSms(Stream Parameter)
        {
            if (!IsValid) return responceDispatcher();
        ...
        public Stream ExecuteDataService(Stream Parameter)
        {
            ...
            return responceDispatcher(data);
        }
        #endregion

        #region Dev methods Works only on debug library
#if DEBUG
        public Stream DevQuery(Stream Parameter)
        {
            if (IsValid)
            {
                try
